...
export default function BottomRightCorner() {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-0 right-0 p-3 pt-5 pl-8 sm:p-4 sm:pt-6 sm:pl-10 md:p-6 md:pt-8 md:pl-14 bg-base-100 rounded-tl-[1.5rem] sm:rounded-tl-[2rem] md:rounded-tl-[3.5rem] flex items-center gap-3 sm:gap-4 md:gap-6"
    >
      {/* Mask góc trên */}
      ...
      <div className="bg-hero-ink/5 w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center border border-hero-ink/10">
        <ArrowUpRight className="w-5 h-5 md:w-6 md:h-6 text-hero-ink/80" />
      </div>
      <div className="flex flex-col">
        <span className="text-[16px] md:text-[20px] font-normal text-hero-ink/95">{t('hero.exploreCollection')}</span>
        <Link to="/shop" className="flex items-center gap-1 text-hero-ink/60 cursor-pointer hover:text-hero-ink/80 transition-colors">
          <span className="text-[12px] md:text-[15px] font-normal">{t('hero.viewShop')}</span>
